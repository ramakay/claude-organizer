...
async function addPattern(patterns: string[]) {
  ...
  try {
    ...
    if (!currentSkipPatterns) {
      console.log(
        'ℹ️  No custom skip patterns set. Cannot remove from default patterns.'
      )
      console.log(
        '💡 To customize skip patterns, first set CLAUDE_ORGANIZE_SKIP_PATTERNS in .env'
      )
      console.log(
        '   Example: CLAUDE_ORGANIZE_SKIP_PATTERNS=README.md,LICENSE,*.log'
      )
      process.exit(1)
    }

    // Parse current patterns
    const currentPatternsList = currentSkipPatterns
      .split(',')
      .map((p) => p.trim())
      .filter((p) => p)
